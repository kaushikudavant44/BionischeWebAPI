...
public interface LabAppointmentDetailsRepository extends JpaRepository<LabAppointmentDetails, Integer>{
	...
	@Query(value="SELECT a.* FROM t_lab_appointment_details a, appointment_time t WHERE a.status=1 AND a.lab_app_date = CURDATE() AND t.time_id IN (:timeIdList) AND a.time_id=t.time_id",nativeQuery=true)
