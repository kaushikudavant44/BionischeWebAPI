...
public interface GetDoctorHospitalDetailsRepository extends JpaRepository<GetDoctorHospitalDetails, Integer>{
	...
	@Query(value="SELECT d.id,d.hospital_id, d.del_status, d.doctor_id, h.hospital_name, h.type from doctor_hospital_details d, hospital_details h where d.doctor_id=:doctorId AND d.hospital_id=h.hospital_id AND d.del_status=:delStatus",nativeQuery=true)
