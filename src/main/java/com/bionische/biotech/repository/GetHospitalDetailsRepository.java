...
public interface GetHospitalDetailsRepository extends JpaRepository<GetHospitalDetails, Integer>{
	...
	@Query(value="SELECT d.id,h.type, d.doctor_id, d.hospital_id, h.hospital_name, h.cityid, c.city_name, h.state_id, s.state_name, h.email_id, h.address, h.contact_no\r\n" + 
			...
			"WHERE doctor_id=:doctorId AND d.hospital_id=h.hospital_id AND h.cityid=c.city_id AND h.state_id=s.state_id AND h.del_status=0 AND d.del_status=0;",nativeQuery=true)
	...
	@Query(value="SELECT d.id, h.type, d.doctor_id, h.hospital_name, h.contact_no, h.email_id, \r\n" + 
			...
			"FROM doctor_hospital_details d, hospital_details h, city c, state s WHERE h.hospital_id=:hospitalId AND h.cityid=c.city_id AND h.state_id=s.state_id AND h.del_status=0;",nativeQuery=true)
