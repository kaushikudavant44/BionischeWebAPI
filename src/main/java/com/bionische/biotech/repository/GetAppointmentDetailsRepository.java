...
public interface GetAppointmentDetailsRepository extends JpaRepository<GetAppointmentDetails, Integer>{
	...
	@Query(value="SELECT a.appoint_id,a.int_1, a.doctor_id, a.patient_id, a.date, a.hospital_id, a.del_status, a.status, CONCAT(p.f_name,' ', p.l_name)AS patient_name, a.amount,"
	...
	@Query(value="SELECT a.appoint_id, a.doctor_id, a.patient_id, a.date, a.hospital_id, a.int_1,a.del_status, a.status, CONCAT(p.f_name,' ', p.l_name)AS patient_name,a.amount,"
			+" CONCAT(d.f_name,' ', d.l_name)AS doctor_name, h.address, h.hospital_name, t.time_id,t.time,p.profile_photo from appointment_time t, doctor_appointment a, doctor_details d,"
