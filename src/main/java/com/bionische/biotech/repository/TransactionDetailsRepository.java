...
public interface TransactionDetailsRepository extends JpaRepository<TransactionDetails, Integer>{
	...
	@Transactional
	@Modifying
	@Query("UPDATE TransactionDetails   SET  int_2=:receiptNo WHERE id IN(:reportId)")
	int updateReportReceiptStatusAndReceiptNo(@Param("reportId")List<Integer> reportId, @Param("receiptNo")int receiptNo);
