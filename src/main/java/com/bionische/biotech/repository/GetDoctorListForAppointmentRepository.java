...
public interface GetDoctorListForAppointmentRepository extends JpaRepository<GetDoctorListForAppointment, Integer>{
		...
		@Query(value="SELECT d.doctor_id, d.f_name, d.l_name, d.m_name, d.city_id, d.address, d.year_of_exp, d.about_me,"
				...
				+ " AND ds.package_exp_date>=:date AND ds.txn_status=1 AND d.doctor_id NOT IN(SELECT l.doctor_id FROM t_doctor_leaves l  WHERE d.doctor_id=l.doctor_id AND l.date=:date) GROUP BY a.doctor_id ",nativeQuery=true)
	List<GetDoctorListForAppointment> getDoctorListForAppointment(@Param("specId")int specId, @Param("cityId")int cityId, @Param("date")String date);
		...
		@Query(value="SELECT d.doctor_id, d.f_name,d.l_name, d.m_name, d.city_id, d.address, d.year_of_exp, d.about_me, d.fees, a.hospital_id,"
				...
				+ " d.doctor_id=ds.doctor_id AND ds.package_exp_date>=:date AND ds.txn_status=1   AND a.del_status=0 AND a.doctor_id NOT IN (:doctoridList) AND d.doctor_id NOT IN(SELECT l.doctor_id FROM t_doctor_leaves l  WHERE d.doctor_id=l.doctor_id AND l.date=:date) GROUP BY a.doctor_id",nativeQuery=true) 
