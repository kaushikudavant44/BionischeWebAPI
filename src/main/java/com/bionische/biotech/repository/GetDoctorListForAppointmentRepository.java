...
public interface GetDoctorListForAppointmentRepository extends JpaRepository<GetDoctorListForAppointment, Integer>{
	...
	List<GetDoctorListForAppointment> getDoctorListForAppointment(@Param("specId")int specId, @Param("cityId")int cityId, @Param("date")String date);
		...
		@Query(value="SELECT d.doctor_id, d.f_name,d.l_name, d.m_name, d.city_id, d.address, d.year_of_exp, d.about_me, d.fees, a.hospital_id,"
				...
				+ " s.spec_type, d.council_reg_no, d.council_name, COALESCE((SELECT ((SUM(r.rating)*5)/(COUNT(r.rating_review_id)*5))  FROM rating_details r"
				...
				+ " d.doctor_id=ds.doctor_id AND ds.package_exp_date>=NOW() AND ds.txn_status=1 AND a.doctor_id NOT IN (:doctoridList) GROUP BY a.doctor_id",nativeQuery=true) 
		List<GetDoctorListForAppointment> getDoctorListForAppointment(@Param("specId")int specId, @Param("cityId")int cityId, @Param("doctoridList")List<Integer> doctoridList);
}
