...
public interface ConsultingDetailsRepository extends JpaRepository<ConsultingDetails, Integer> {
	...
	@Query(value="select m.meet_id,d.doctor_id,h.contact_no,h.email_id,p.email, m.int_2,h.address, m.patient_id,CONCAT(p.f_name,' ',p.l_name) as patient_name, h.hospital_name,CONCAT(d.f_name,' ',d.l_name) as f_name,m.time,m.date,m.patient_problem,m.note, m.discussion from hospital_details h, doctor_details d,"
	...
	@Query(value="SELECT m.meet_id,m.doctor_id,m.patient_id,h.contact_no,h.email_id,p.email,h.address,CONCAT(p.f_name,' ',p.l_name) AS patient_name,h.hospital_name, m.int_2, p.f_name,\r\n" + 
	...
	@Query(value="SELECT m.meet_id, m.doctor_id,h.contact_no,h.email_id,p.email, m.patient_id,CONCAT(p.f_name,' ',p.l_name) AS patient_name, h.hospital_name, p.f_name, m.int_2, m.time,h.address, m.date, m.patient_problem,m.note, m.discussion FROM patient_details p,hospital_details h, doctor_details d,doctor_patient_meeting m WHERE  m.patient_id=:patientId AND m.doctor_id=:doctorId AND h.hospital_id=m.int_1 GROUP BY m.meet_id",nativeQuery=true)
