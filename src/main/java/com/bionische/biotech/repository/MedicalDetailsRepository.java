...
public interface MedicalDetailsRepository extends JpaRepository<MedicalDetails, Integer>{
	...
	@Query(value="SELECT * FROM medical_details m,t_pharmacy_subscription_details ms WHERE m.int_3=:pincode and m.del_status=0  AND ms.package_exp_date>=NOW() AND m.medical_id=ms.medical_id AND ms.txn_status=1",nativeQuery=true)
