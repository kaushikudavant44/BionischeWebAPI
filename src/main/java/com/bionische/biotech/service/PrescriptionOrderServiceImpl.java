...
public class PrescriptionOrderServiceImpl implements PrescriptionOrderService {
	...
	public PrescriptionToMedical orderPrescription(PrescriptionToMedical prescriptionToMedical) {
		...
		for (int i = 0; i < prescriptionDetailsList.size(); i++) {
			...
			prescriptionOrderDetails
					.setQuantity(Integer.parseInt(prescriptionDetailsList.get(i).getMedicineQuantity()));
	...
	public List<GetPrescriptionDetailsForOrder> getPrescriptionDetailsForOrder(int requestId) {
		...
		try {
			getPrescriptionDetailsForOrderList = getPrescriptionDetailsForOrderRepository
					.getPrescriptionDetailsForOrderByRequestIdId(requestId);
		} catch (Exception e) {
			System.out.println(e.getMessage());// TODO: handle exception
		}
	...
	public Info updateMedicalOrderStatusAndAmount(int requestId, int status, float totAmount) {
		...
		try {
			...
			if (res > 0) {
				...
				sendEMailService.sendMail("Your Order is Ready", getPatientContactDetailsById.getfName() + " "
						+ getPatientContactDetailsById.getlName()
						+ " Your order is Ready. If your order is Home delivery then your order delivered soon. \n Your payable amount is : Rs. "
						+ totAmount + " You can pay your order bill online by click on following link. "
						+ ConstantFileUploadPath.FRONTEND_URL
						+ "showMyOrder/ \n We will notify you about order status\n Thank you.",
						getPatientContactDetailsById.getEmail());

				String confirmMedicineOrderNoti = patientDetails.getfName() + " " + patientDetails.getlName()
						+ " your medicine order is ready check your medicine bill";

				sendFcmNotificationService.notifyUser(patientDetails.getString2(), "BIONISCHE",
						confirmMedicineOrderNoti, DateConverter.currentDateAndTime(), 16);
				info.setError(false);
	...
	public Info updateMedicineOrderPaidStatus(int requestId, int paidStatus) {
		...
		try {
			...
			PrescriptionToMedical prescriptionToMedical=prescriptionToMedicalRepository.findByRequestToMedicalId(requestId);
			PatientDetails patientDetails=patientDetailsRepository.findByPatientId(prescriptionToMedical.getPatientId());
			if (res > 0) {
				info.setError(false);
				info.setMessage("Order Status update successfully");
				String completeOrderNot=patientDetails.getfName()+" "+patientDetails.getlName()+" transaction complete successfuly";
				sendFcmNotificationService.notifyUser(patientDetails.getString2(), "BIONISCHE", completeOrderNot, DateConverter.currentDateAndTime(),19);
	...
	public Info updateMedicineOrderDeliveredStatus(int requestId, int status) {
		...
		try {
			...
			if (res > 0) {
				...
				PatientDetails patientDetails=patientDetailsRepository.findByPatientId(patientId);
				String orderDelivredNotification=getPatientContactDetailsById.getfName() + " " + getPatientContactDetailsById.getlName()
				+ " Your order is Deliverd successfully. " + ConstantFileUploadPath.FRONTEND_URL
				+ "showMyOrder/ \n  Thank you.";
				sendFcmNotificationService.notifyUser(patientDetails.getString2(), "BIONISCHE", orderDelivredNotification, DateConverter.currentDateAndTime(),20);
				sendEMailService.sendMail("Your Order is Delivered Successfully",
						getPatientContactDetailsById.getfName() + " " + getPatientContactDetailsById.getlName()
								+ " Your order is Deliverd successfully. " + ConstantFileUploadPath.FRONTEND_URL
								+ "showMyOrder/ \n  Thank you.",
						getPatientContactDetailsById.getEmail());
	...
	public List<GetMedicalOrderDetails> getMedicalOrderDetailsByMedicalIdAndStatusAndDate(int medicalId, int status,
		...
		List<GetMedicalOrderDetails> getMedicalOrderDetailsList = new ArrayList<GetMedicalOrderDetails>();
		try {
			getMedicalOrderDetailsList = getMedicalOrderDetailsRepository
					.getMedicalOrderDetailsByMedicalIdAndStatusAndDate(medicalId, status, fromDate, toDate);
	...
	@Override
	public List<GetMedicalOrderDetails> getMedicalOrderDetailsByMedicalIdAndDate(int medicalId, String fromDate,
			String toDate) {
		List<GetMedicalOrderDetails> getMedicalOrderDetailsList = new ArrayList<GetMedicalOrderDetails>();
		try {
			getMedicalOrderDetailsList = getMedicalOrderDetailsRepository
					.getMedicalOrderDetailsByMedicalIdAndDate(medicalId, fromDate, toDate);
	...
	@Override
	public List<GetMedicalOrderDetails> getMedicalOrderDetailsByPatientIdAndDate(int patientId, String fromDate,
			String toDate) {

		List<GetMedicalOrderDetails> getMedicalOrderDetailsList = new ArrayList<GetMedicalOrderDetails>();
		try {
			getMedicalOrderDetailsList = getMedicalOrderDetailsRepository
					.getMedicalOrderDetailsByPatientIdAndStatusAndDate(patientId, fromDate, toDate);
	...
	public List<GetMedicalOrderDetails> getMedicalCancelledOrderByMedicalIdAndStatus(int medicalId, int status) {
		...
		try {
			getMedicalOrderDetailsList = getMedicalOrderDetailsRepository
					.getMedicalCancelledOrderByMedicalIdAndStatus(medicalId, status);
