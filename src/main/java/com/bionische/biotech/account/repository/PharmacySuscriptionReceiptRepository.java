...
public interface PharmacySuscriptionReceiptRepository extends JpaRepository<PharmacySuscriptionReceipt, Integer>{
	...
	@Query(value="select  s.suscription_id, s.medical_id, s.amount, s.order_id, s.created_date, c.gst_no as company_gst_no, d.acc_holder_name as pharmacy_name,"
	...
	List<PharmacySuscriptionReceipt> getPharmacySuscriptionReceipt(@Param("medicalId")int medicalId);

	@Query(value="select  s.suscription_id, s.medical_id, s.amount, s.order_id, s.created_date, c.gst_no as company_gst_no, d.acc_holder_name as pharmacy_name,"
			+ " s.txn_id, s.package_amt, d.gst_no, c.contact_no1, c.contact_no2, c.email, c.website, c.address, "
			+ "c.company_name from m_bionische_company_info c, t_pharmacy_subscription_details s, pharmacy_bank_info d where s.suscription_id=:suscriptionId",nativeQuery=true)
	PharmacySuscriptionReceipt getPharmacySuscriptionReceiptBySubscriptionId(@Param("suscriptionId")int suscriptionId);
