...
public interface DoctorSuscriptionReceiptRepository extends JpaRepository<DoctorSuscriptionReceipt, Integer>{
	...
	@Query(value="select  s.suscription_id, s.doctor_id, s.amount, s.order_id, s.created_date, c.gst_no as company_gst_no, d.acc_holder_name as doctor_name,"
	...
	@Query(value="select  s.suscription_id, s.doctor_id, s.amount, s.order_id, s.created_date, c.gst_no as company_gst_no, d.acc_holder_name as doctor_name,"
			+ " s.txn_id, s.package_amt, d.gst_no, c.contact_no1, c.contact_no2, c.email, c.website, c.address, "
			+ "c.company_name from m_bionische_company_info c, t_doctor_subscription_details s, doctor_bank_info d where d.doctor_id=s.doctor_id  AND s.txn_status=1 AND s.suscription_id=:suscriptionId",nativeQuery=true)
	DoctorSuscriptionReceipt findBySuscriptionId(@Param("suscriptionId")int suscriptionId);
