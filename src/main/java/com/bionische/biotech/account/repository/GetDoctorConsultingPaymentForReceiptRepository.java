...
public interface GetDoctorConsultingPaymentForReceiptRepository extends JpaRepository<GetDoctorConsultingPaymentForReceipt, Integer>{
	...
	@Query(value="select t.appoint_id, t.doctor_id, t.amount, t.txn_id, t.payment_date, t.patient_id from  doctor_appointment t where t.payment_date between :fromDate AND :toDate AND t.doctor_id=:doctorId AND t.payment_status=1 AND t.int_2=0 order by t.payment_date ASC",nativeQuery=true)
