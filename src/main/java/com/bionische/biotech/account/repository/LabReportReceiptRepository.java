...
public interface LabReportReceiptRepository extends JpaRepository<LabReportReceipt, String>{
	...
	@Query(value="SELECT COALESCE((SELECT r.date FROM t_lab_report_receipt r WHERE r.lab_id=:dlabId ORDER BY r.receipt_no DESC LIMIT 1),(SELECT a.payment_date FROM transaction a WHERE a.to_user_id=:labId AND a.status=1 ORDER BY a.payment_date ASC LIMIT 1))AS date",nativeQuery=true)
